...
def normalize_entity_labels(entities: list[dict]) -> list[dict]:
    """Normalize the label of every entity in one document's entity list."""
    return [{**entity, "label": normalize_label(entity["label"])} for entity in entities]
...
def normalize_annotation_labels(
    ...
    output[label_column] = output[label_column].map(normalize_label)
