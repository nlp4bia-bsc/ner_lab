...
def _validate_stats(stats: str, base_model: str | None, language: str | None) -> None:
    if stats not in STATS_CHOICES:
        raise ValueError(f"stats must be one of {STATS_CHOICES}, got {stats!r}.")

    if stats != "none" and (base_model is None or language is None):
        raise ValueError("stats requires both base_model and language.")
...
def _write_stats(
    ...
    compute_text_stats(corpus, base_model, language, output_dir=dataset_root)

    if stats == "both":
        compute_annotation_stats(
            corpus, base_model, normalize_labels=normalize_labels, output_dir=dataset_root
        )
