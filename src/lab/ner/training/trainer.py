...
class CRFTrainer(Trainer):
    ...
    def prediction_step(
        ...
        from lab.ner.models.crf import decoded_paths_to_logits
...
def train(
    ...
    `lab.ner.training.training_arguments` for this library's defaults, or pass
    your own.
    ...
    one with `lab.ner.training.model_encoding`; without it a saved directory
    cannot be loaded by `lab.ner.inference` — and a CRF model, which the
...
def _check_best_metric(training_arguments: TrainingArguments, compute_metrics) -> None:
    ...
    raise ValueError(
        ...
        "Build one with lab.ner.evaluation.build_compute_metrics, or set "
